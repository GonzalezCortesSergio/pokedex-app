...
export class PokemonModalComponent {
    ...
    getPokemonDescription(): void {
        const speciesUrlParts = this.pokemon.species.url.split('/'); 
        const pokemonId = parseInt(speciesUrlParts[speciesUrlParts.length - 2], 10); 
    
        this.pokemonsService.getPokemonDescription(pokemonId).subscribe(data => {
            const entry = data.flavor_text_entries.find(
                (entry: any) => entry.language.name === 'es' && entry.version.name === 'black'
            );
            this.description = entry ? entry.flavor_text : 'Descripción no disponible';
        });
    }
